...
async function createGuestAccount({ selectedAvatar, selectedGrade, signUp }: GuestAccountParams, retryCount = 0): Promise<any> {
  try {
    ...
    try {
      const response = await fetch(`${HOST_URL}/public/learn/learner/create`, {
        ...
        body: JSON.stringify({
          ...
          school_address: "Guest Address",
          school_latitude: 0,
          school_longitude: 0,
          terms: "1,2,3,4", // Default terms
          curriculum: "CAPS", // Default curriculum
    ...
    if (retryCount < MAX_RETRIES) {
      ...
      return createGuestAccount({ selectedAvatar, selectedGrade, signUp }, retryCount + 1);
    }
    throw error;
  }
}
...
export default function OnboardingScreen() {
  ...
  const handleComplete = async () => {
    try {
      ...
      await analytics.track('Maths Normal Account Created', {});
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    ...
                    await analytics.track('Maths Guest Account Created', {});
                    router.replace('/(tabs)');
                  } catch (error) {
